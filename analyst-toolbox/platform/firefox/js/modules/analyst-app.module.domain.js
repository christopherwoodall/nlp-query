...
Domain.filter = function( text ) {
  // TODO - better domain filtering; pos. move vars to global so they are not redeclared.
  //        verify code is flushed out of memory
  //https://github.com/johno/domain-regex
  if( text.indexOf( '.com' ) === -1 ) return -1;
  //https://www.oreilly.com/library/view/regular-expressions-cookbook/9781449327453/ch08s15.html
  let match = text.match( /\b((?=[a-z0-9-]{1,63}\.)(xn--)?[a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,63}\b/gmi )
...
Domain.transform = function( node, match ) {
  node.setAttribute( 'data-analyst-toolbox-tooltip', match )
  return node
}

/******************************************************************************/

Domain.callback = function( event ) {
  var target = this.target = event.target
  var match  = this.match  = target.getAttribute( 'data-analyst-toolbox-match' )
  Domain.toggleModal( match )
}

/******************************************************************************/

Domain.whois = function( match ) {
  // Remove any subdomains
  var whois    = { },
      response = { },
      domain   = match.split( '.' ).slice( -2 ).join( '.' ),
  ...
  xhttp.onreadystatechange = function() {
    if( this.readyState == 4 && this.status == 200 ) {
      ...
      //whois.nameservers = response.nameserver.map( ns => ns.ldhName );
      whois.status  = response.status.join( `<br />` );
      whois.expires = response.events[0].eventDate;
      whois.registered = response.events[1].eventDate;
      //whois.registrant = response.
      //whois.registar = response.
      //console.log( response )
    }
  };
  xhttp.open("GET", `https://rdap.markmonitor.com/rdap/domain/${domain}`, false);
...
Domain.toggleModal = function( match ) {
  var modal = document.getElementById( 'analyst-toolbox' ),
      whois = Domain.whois( match )
  const modal_content = `
  ...
  <h3>External Tools</h3>
    ...
    <a href="https://otx.alienvault.com/indicator/domain/${match}" target="_blank" rel="noopener noreferrer">
      Aliven Vault
    </a>

    <a href="https://www.virustotal.com/en/domain/${match}/information/" target="_blank" rel="noopener noreferrer">
      Virus Total
    </a>

    <a href="https://bgp.he.net/dns/${match}" target="_blank" rel="noopener noreferrer">
      Hurricane Electric
    </a>
  </div>
  </div>
...
/**
 * NOTES
 *   o Valid TLDs - https://data.iana.org/TLD/tlds-alpha-by-domain.txt
